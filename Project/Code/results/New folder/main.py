    ...
    def predict(self, mode, path):
        ...
        for key, value in self.test_classes.items():
            for doc in value.documents:
                itr +=1
                status1 = self.assign_class(doc, 'U')
                status2 = self.assign_class(doc, 'B')
                status3 = self.assign_class(doc, 'S')
                m1 =0
                m2 =0
                m3 =0
                #print(status1,status2,status3)
                #print(type(status1))
                for x in [status1,status2,status3]:
                    if x=="ferdowsi":
                        m1 = m1+1
                    elif x=="hafez":
                        m2 = m1+1
...
if __name__ == "__main__":
    ...
    #print(accuracy)
    plt.figure()
    plt.plot([x+1 for x in range(10)] ,accuracy,'-', label='hard majority voting')
    #plt.xlabel("Iteration")
    #plt.ylabel("Accuracy (%)")  
    #plt.ylim((0,100))
    #plt.title('Multi-Classifier Fusion')
    ...
    for i in range(10):
        model1 = NLP1()  # Initiating the model
        model1.train('train_set')
        file_name = 'test_file' + str(i+1) +'.txt'
        print(file_name)
        accuracy.append(100*model1.binary_back_off_predict('test_set/' + file_name))
        model1.print_table()
    #print(accuracy)
    #plt.figure()
    plt.plot([x+1 for x in range(10)] ,accuracy,'-', label='soft majority voting')
    plt.xlabel("ITERATION")
    plt.ylabel("ACCURACY (%)")  
    plt.ylim((0,100))
    plt.title('Multi-Classifier Fusion')
