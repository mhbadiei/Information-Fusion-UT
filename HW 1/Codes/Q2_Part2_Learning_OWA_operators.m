...
orness = 0;
for i=1:1:m_sensor-1
        orness = orness + (1./((m_sensor-1)-1))*(((m_sensor-1)-i)*w(i));  
end
orness;
...
hold on;
plot(A(:,4))
title('Estimated and Real mesures in the same figure')
xlabel('iteration (observation)')
ylabel('Learning OWA Output from observation')
legend('Estimated mesure','Real mesure')